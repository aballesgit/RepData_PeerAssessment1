...
The mean of the total number of steps taken per day is `r mean_steps` and the median of the total number of steps taken per day is `r median_steps`.
...
The 5-minute interval that contains the maximum number of steps is `r interv_max` - `r interv_max2`.
...
The total number of missing values in the dataset is `r rows_na`.

```{r}
days_na <- unique(na$date)
```
The days which have missing values are: `r days_na`.
...
Now, the mean of the total number of steps taken per day is `r new_mean_steps` and the median of the total number of steps taken per day is `r new_median_steps`.

The mean is the same as the median now, whereby before the mean was a little smaller than the median. The difference between the median now minus the median before is: `r diff` steps, which accounts for `r perc`% of the initial median.
